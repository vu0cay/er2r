...
from CGTRINH CT join CHUTHAU C on C.TEN_THAU = CT.TEN_THAU
where C.TEN_THAU like 'cty xd so 6';
--c6
select DISTINCT TEN_THAU, c.DCHI_THAU
from KTRUCSU K join THIETKE T on K.HOTEN_KTS = T.HOTEN_KTS
    join CGTRINH CT on CT.STT_CTR = T.STT_CTR
        join CHUTHAU C on C.TEN_THAU = CT.TEN_THAU
where K.HOTEN_KTS like 'le kim dung' and TINH_THANH like 'can tho';
--c7
select noi_tn
from ktrucsu k join thietke t on k.hoten_kts = t.hoten_kts 
    join cgtrinh ct on ct.stt_ctr = t.stt_ctr
where ten_ctr like 'khach san quoc te' and tinh_thanh like 'can tho';
--c8
select c.hoten_cn, nams_cn, nam_vao_n
from congnhan c join thamgia t on c.hoten_cn = t.hoten_cn
    join cgtrinh ct on ct.stt_ctr = t.stt_ctr
    join chuthau cthau on cthau.ten_thau = ct.ten_thau
where ch_mon like 'han' or ch_mon like 'dien' 
    and cthau.ten_thau like 'le van son';
--c9
select c.*
from congnhan c join thamgia t on c.hoten_cn = t.hoten_cn
    join cgtrinh ct on ct.stt_ctr = t.stt_ctr
...
--c10
select  DISTINCT k.hoten_kts, nams_kts
from ktrucsu k join thietke t on k.hoten_kts = t.hoten_kts 
    join cgtrinh ct on ct.stt_ctr = t.stt_ctr
where noi_tn like 'tp hcm' and kinh_phi > 400;
--c11
select c.hoten_cn, ch_mon
from congnhan c join thamgia t on c.hoten_cn = t.hoten_cn
    join cgtrinh ct on ct.stt_ctr = t.stt_ctr
    join thietke tk on tk.stt_ctr = ct.stt_ctr 
where hoten_kts like 'le thanh tung';
--c12
select ten_ctr
from cgtrinh
group by ten_ctr, kinh_phi having kinh_phi = (
    select max(kinh_phi) k from cgtrinh);
--c13
select hoten_kts, nams_kts
from ktrucsu
group by hoten_kts, nams_kts having nams_kts = (
    select max(nams_kts) k from ktrucsu);
--c14
select c.ten_thau, sum(kinh_phi) 
from chuthau c join cgtrinh ct on c.ten_thau = ct.ten_thau
group by c.ten_thau;
--c15
...
from chuthau c join cgtrinh ct on c.ten_thau = ct.ten_thau
where kinh_phi = (
    select min(kinh_phi) m from cgtrinh
    );
--c16
create table c16 as
    select hoten_kts, sum(thu_lao) s
    from thietke
    group by hoten_kts having sum(thu_lao) > 25;
select * from c16;
--c17
select count(hoten_kts) 
from c16;
--c18
...
--c19
create table c19 as
    select stt_ctr, count(hoten_cn) socongnhan
    from thamgia
    group by stt_ctr;
select * from c19;
--c20
select ten_ctr, diachi_ctr
from c19 c join cgtrinh ct on c.stt_ctr = ct.stt_ctr
where socongnhan = (
    select max(socongnhan) from c19);
--c21
